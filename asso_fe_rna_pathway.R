...
fer_1M_nested <- fer_1M_nested %>% 
  ...
  mutate(result = map(data, ~run_path_1(dat.x = .x, dat.y = dat.rna.1M, sequencing_depth = sequencing_depth_1M))) %>% 
  collect() %>% 
  as_tibble()
...
fer_2M_nested <- fer_2M_nested %>% 
  ...
  mutate(result = map(data, ~run_path_2(dat.x = .x, dat.y = dat.rna.2M, sequencing_depth = sequencing_depth_2M))) %>% 
  collect() %>% 
  as_tibble()
...
fer_3M_nested <- fer_3M_nested %>% 
  ...
  mutate(result = map(data, ~run_path_1(dat.x = .x, dat.y = dat.rna.3M, sequencing_depth = sequencing_depth_3M))) %>% 
  collect() %>% 
  as_tibble()
...
fer_6M_nested <- fer_6M_nested %>% 
  ...
  mutate(result = map(data, ~run_path_3(dat.x = .x, dat.y = dat.rna.6M, sequencing_depth = sequencing_depth_6M))) %>% 
  collect() %>% 
  as_tibble()
